...
RSpec.describe Enigma do
  ...
  it '#encrypt a message (generates random key and uses todays date)' do
    ...
    expect(encrypt_no_key[:keys]).to be_a(String)
    expect(encrypt_no_key[:keys].length).to eq(5)
    expect((0..99_999).include?(encrypt_no_key[:keys].to_i)).to eq(true)
  end
  ...
  it '#crack and encryption with a date' do
    enigma = Enigma.new
    allow(Keys).to receive(:random_key).and_return('08304')
  ...
  it '#crack an encryption (uses todays date)' do
    enigma = Enigma.new
    allow(Keys).to receive(:random_key).and_return('08304')
    allow(Offsets).to receive(:current_date).and_return('291018')
